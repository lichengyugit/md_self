...
class OrderModel extends DB_Model
{
     ...
     * 根据条件获得所有订单列表
     */
    public function getAllOrder($parames){
        $where="";
        foreach ($parames as $k=>$v){
            $where.=" AND ".$k." = ".$v;
        }
...
//        empty($parames['pay_status']) || $where.=" AND O.pay_status=".$parames['pay_status'];       $whereAttr.=" AND pay_status=".$parames['pay_status'];
        ...
        if(isset($parames['input_data']) && $parames['input_data']!=''){
            ...
            if(preg_match('/^[\x7f-\xff]+$/',$inputData)){
                ...
                $whereAttr.=" AND  user_name LIKE "."'%". $inputData ."%'";
    ...
    * */
     ...
     * 根据条件获得所有订单列表
     */
    public function getAllOrders($parames){
        $where="";
        foreach ($parames as $k=>$v){
            $where.=" AND ".$k." = ".$v;
        }
     ...
     * 用户端获取个人信息
     * */
    public function getOrderAll($parames)
    {
        $sql = " SELECT id,order_sn,pay,rent_time,vehicle_id,vehicle_number,vehicle_brand,merchant_name,merchant_id,begin_time,end_time,order_status,create_time,pay_time,extend_time
        FROM  ".$this->tablename." WHERE `status` = 0 AND pay_status in (1,2) AND order_status in (1,2)  AND user_id=".$parames['user_id']." ORDER BY id DESC";
        return $this->getCacheResultArray($sql);
    }
}
