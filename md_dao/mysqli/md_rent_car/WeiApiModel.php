...
class WeiApiModel extends DB_Model {
    ...
    /**
     * 魔力后台导入用户信息
     * @param $userArr 更新用户表数据的数组 array
     * @param $cardArr 插入月卡表数据的数组 array
     */
    public function synchroUser($userArr,$cardArr){
        $this->write_db->trans_begin();
        try{
            //更新用户表
            $user = M_Mysqli_Class('md_lixiang', 'UserModel')->updateUser($userArr);
            //如果更新失败抛出异常
            if(!$user){
            ...
            if(count($cardArr)){
                //新增用户月卡记录
                $card=M_Mysqli_Class('md_lixiang','UserCardModel')->insertUserCard($cardArr);
                
                if(!$card){
                    throw new \Exception('月卡表插入失败');
                } 
            }
            $this->write_db->trans_commit();
            return true;
