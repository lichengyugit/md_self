...
class UserWalletLogModel extends DB_Model {
    ...
    {
        parent::__construct('md_rent_car', 'md_user_wallet_log');
    ...
    * */
    ...
    /**
     * 根据条件获取商户提现和分佣的钱包日志(接口用)
     */
    public function getWalletLogsByAttr($parames){
        $where="";
        foreach ($parames as $k=>$v){
            $where.=" AND ".$k." = '".$v."'";
        }
        $sql = " SELECT * FROM ".$this->tablename." WHERE `type` in(2,3) ".$where." ORDER BY id DESC";
        return $this->getCacheResultArray($sql);
    }
}
?>