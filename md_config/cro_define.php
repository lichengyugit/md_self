...
class DefineConfig {
    ...
    private function setEnvironmentAndDomain() {
        ...
        if ($hostArray[0] == $this->abName) {
            ...
            define('WEIAPI_URL', PROTOCOL . '://' . ENVIRONMENT . '.weiapi.' . DOMAIN_HOST );
            define('LIXIANG_URL', PROTOCOL . '://' . ENVIRONMENT . '.lixiang.' . DOMAIN_HOST );
            define('PAY_URL', PROTOCOL . '://' . ENVIRONMENT . '.pay.' . DOMAIN_HOST );
            //define('PIC_URL', PROTOCOL . '://' . ENVIRONMENT . '.pic.' . DOMAIN_HOST );
            define('ML_URL', PROTOCOL.'://load.'.DOMAIN_HOST.'/TESTDogWood_api/wechat/wechat/');
            error_reporting(0);
        }elseif(filter_var($_SERVER['REMOTE_ADDR'], FILTER_VALIDATE_IP)!=false){
            define('ENVIRONMENT', 'dev');
            ...
            define('PIC_URL', PROTOCOL . '://pic.' . DOMAIN_HOST);
            define('SCAN_URL', PROTOCOL . '://scan.' . DOMAIN_HOST . ':8080');
            define('WEIAPI_URL', PROTOCOL . '://weiapi.' . DOMAIN_HOST );
            define('LIXIANG_URL', PROTOCOL . '://lixiang.' . DOMAIN_HOST );
            error_reporting(E_ALL);
        } else {
