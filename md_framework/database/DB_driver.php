...
abstract class CI_DB_driver {
    ...
    public function __construct($params) {
        ...
        $this->log->log_trace('数据驱动类实例化 Database Driver Class Initialized');
    }
    ...
    public function initialize() {
        ...
        if (!$this->conn_id) {
            ...
            if (!$this->conn_id) {
                $this->log->log_error('不能连接数据库 Unable to connect to the database');
    ...
    public function db_set_charset($charset) {
        if (method_exists($this, '_db_set_charset') && !$this->_db_set_charset($charset)) {
            $this->log->log_error('不能设置数据库字符集 Unable to set database connection charset: ' . $charset);
    ...
    public function query($sql, $binds = FALSE, $return_object = NULL) {
        if ($sql === '') {
            $this->log->log_error('无效的语句 Invalid query: ' . $sql);
        ...
        if (FALSE === ($this->result_id = $this->simple_query($sql))) {
            ...
            // Log errors
            $this->log->log_error('语句错误 Query error: ' . $error['message'] . ' - 无效的语句 Invalid query: ' . $sql);
    ...
    public function load_rdriver() {
        ...
        if (!class_exists($driver, FALSE)) {
            require_once (DEFAULT_SYSTEM_PATH . 'database/DB_result.php');
            require_once (DEFAULT_SYSTEM_PATH . 'database/drivers/' . $this->dbdriver . '/' . $this->dbdriver . '_result.php');
        }
    ...
    public function trans_complete() {
        ...
        if ($this->_trans_status === FALSE or $this->_trans_failure === TRUE) {
            ...
            $this->log->log_trace('DB Transaction Failure 数据库事务失败');
            return FALSE;
        }
     ...
     * @param bool $like not the string will be used in a LIKE condition
     * @return string
     */
